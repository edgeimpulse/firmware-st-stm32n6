...
/**
  ...
  */
__attribute__((naked)) void boot_jump_to_image(uint32_t boot_jump_addr, uint32_t reset_handler_addr)
{
  __ASM volatile(
#if !defined(__ICCARM__)
    ".syntax unified                 \n"
...
{
  struct boot_arm_vector_table *vt = (struct boot_arm_vector_table *)vector;
  ...
  tmp_vt -= PRIMARY_PARTITION_START;     /* code offset in primary partition */
  tmp_vt += PRIMARY_RUN_PARTITION_START; /* address in primary run partition */
