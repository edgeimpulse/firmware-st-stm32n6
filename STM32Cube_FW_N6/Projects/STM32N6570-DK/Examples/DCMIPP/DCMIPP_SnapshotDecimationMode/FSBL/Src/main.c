...
int main(void)
{
  ...
  if (HAL_DCMIPP_CSI_PIPE_Start(&phdcmipp, DCMIPP_PIPE1, DCMIPP_VIRTUAL_CHANNEL0 , BUFFER_ADDRESS, DCMIPP_MODE_SNAPSHOT) != HAL_OK)
  {
    Error_Handler();
  }
  ...
  HAL_DCMIPP_PIPE_SetDecimationConfig(&phdcmipp, DCMIPP_PIPE1, &pDecConfig);
  HAL_DCMIPP_PIPE_EnableDecimation(&phdcmipp, DCMIPP_PIPE1);
  HAL_DCMIPP_PIPE_SetPitch(&phdcmipp, DCMIPP_PIPE1, pitch_value);

  /* Update LTDC Display window */
  HAL_LTDC_SetWindowSize(&phltdc, FRAME_WIDTH/2 , FRAME_HEIGHT/2, LTDC_LAYER_1);
  ...
  /* Update LTDC Display window */
  HAL_LTDC_SetWindowSize(&phltdc, FRAME_WIDTH/4 , FRAME_HEIGHT/4, LTDC_LAYER_1);
...
static void LCD_Init(uint32_t Width, uint32_t Height)
{
  ...
  phltdc.Init.Backcolor.Green = 0x0;
