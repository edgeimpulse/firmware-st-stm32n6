...
HASH_HandleTypeDef handle_hash;
...
int main(void)
{
  ...
  if (HAL_HASH_Start_DMA(&handle_hash, (uint8_t*)aInput, strlen((char const*)aInput), aSHA224Digest) != HAL_OK)
  {
    Error_Handler();
  }
  /* Wait for DMA transfer to complete */ 
  while (HAL_HASH_GetState(&handle_hash) == HAL_HASH_STATE_BUSY);
 ...
 /* Compare computed digest with expected one */
  ...
  /***************************** SHA256 ***************************************/
  /****************************************************************************/
  if(HAL_HASH_DeInit(&handle_hash) != HAL_OK)
  {
    Error_Handler();
  }
  handle_hash.Init.DataType = HASH_BYTE_SWAP;
  handle_hash.Init.Algorithm = HASH_ALGOSELECTION_SHA256;

  if (HAL_HASH_Init(&handle_hash) != HAL_OK)
  {
    Error_Handler();
  }

  /* Start HASH computation using DMA transfer */
  if (HAL_HASH_Start_DMA(&handle_hash, (uint8_t*)aInput, strlen((char const*)aInput), aSHA256Digest) != HAL_OK)
  {
    Error_Handler();
  }
  /* Wait for DMA transfer to complete */ 
...
static void MX_HASH_Init(void)
{
  ...
  handle_hash.Instance = HASH;
  handle_hash.Init.DataType = HASH_BYTE_SWAP;
  handle_hash.Init.Algorithm = HASH_ALGOSELECTION_SHA224;
  if (HAL_HASH_Init(&handle_hash) != HAL_OK)
  {
    Error_Handler();
  }
