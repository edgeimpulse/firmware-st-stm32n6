/**
  ******************************************************************************
  * @file    stm32n6570_discovery_camera_ov5640.c
...
/* Includes ------------------------------------------------------------------*/
#include "stm32n6570_discovery_camera_ov5640.h"
...
#if USE_CAMERA_ISP
static int ISP_AutoExposure(DCMIPP_HandleTypeDef *hdcmipp, uint8_t stage);
#endif
...
static uint32_t Get_PixelPackerPitch(uint32_t Resoloution , uint32_t PixelFormat);
...
int32_t BSP_CAMERA_Init(uint32_t Instance, uint32_t Resolution, uint32_t PixelFormat)
{
  ...
  {
    if ((PixelFormat > CAMERA_PF_YUV422) || (Resolution > CAMERA_R800x480))
    {
      ret = BSP_ERROR_WRONG_PARAM;
    }
    else
...
__weak HAL_StatusTypeDef MX_DCMIPP_Init(DCMIPP_HandleTypeDef *hdcmipp)
{
  ...
  pCSIPipeConf.DataTypeIDB  = DCMIPP_DT_RGB565; /* Don't Care */
  ...
  pPipeConf.PixelPipePitch  = Get_PixelPackerPitch(Camera_Ctx[0].Resolution, Camera_Ctx[0].PixelFormat) ; /* Number of bytes */
...
/**
  * @brief Get PixelPackerPich for Main and Ancillary pipes
  * PixelPackerPitch = pixelnumber*pixeldepth (number of bytes between two consecutive lines)
  * @param Resolution
  * @param PixelFormat
  * @retval pixel_packer_pitch_value
  *
  */
static uint32_t Get_PixelPackerPitch(uint32_t Resolution , uint32_t PixelFormat)
{
  uint32_t pixel_packer_pitch_value;
  uint32_t tmp1 = 0UL;
  uint32_t pixel_depth = 0UL;
  ...
  pixel_packer_pitch_value = tmp1*pixel_depth;
  return pixel_packer_pitch_value;
