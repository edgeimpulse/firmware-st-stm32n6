...
static void CMW_UTILS_get_crop_config_from_manual(CMW_Manual_Configuration_t *conf, DCMIPP_CropConfTypeDef *crop);
...
static void CMW_UTILS_get_scale_configs(DCMIPP_Conf_t *p_conf, float ratio_width, float ratio_height,
                                       DCMIPP_DecimationConfTypeDef *dec, DCMIPP_DownsizeTypeDef *down);
static void CMW_UTILS_get_decimation_config_from_manual(CMW_Manual_Configuration_t *conf,
                                                        DCMIPP_DecimationConfTypeDef *dec);
static void CMW_UTILS_get_downsize_config_from_manual(CMW_Manual_Configuration_t *conf, DCMIPP_DownsizeTypeDef *down);

void CMW_UTILS_GetPipeConfig(uint32_t cam_width, uint32_t cam_height, DCMIPP_Conf_t *p_conf,
                             ...
                             DCMIPP_DownsizeTypeDef *down)
{
  ...
  if (p_conf->mode == CAM_Aspect_ratio_crop)
  {
  ...
  else
  {
    ...
    CMW_UTILS_get_decimation_config_from_manual(&p_conf->manual_conf, dec);
    CMW_UTILS_get_downsize_config_from_manual(&p_conf->manual_conf, down);
...
static void CMW_UTILS_get_crop_config_from_manual(CMW_Manual_Configuration_t *conf, DCMIPP_CropConfTypeDef *crop)
{
  crop->HSize = conf->crop.width;
  crop->VSize = conf->crop.height;
  crop->HStart = conf->crop.offset_x;
  crop->VStart = conf->crop.offset_y;
}
...
static void CMW_UTILS_get_decimation_config_from_manual(CMW_Manual_Configuration_t *conf,
                                                        DCMIPP_DecimationConfTypeDef *dec)
{
  dec->HRatio = CMW_UTILS_get_dec_ratio_from_decimal_ratio(conf->decimation.horizontal_ratio, 0);
  dec->VRatio = CMW_UTILS_get_dec_ratio_from_decimal_ratio(conf->decimation.vertical_ratio, 1);
}

static void CMW_UTILS_get_downsize_config_from_manual(CMW_Manual_Configuration_t *conf, DCMIPP_DownsizeTypeDef *down)
{
  float ratiox = (float)conf->crop.width / (conf->downsize.width * conf->decimation.horizontal_ratio);
  float ratioy = (float)conf->crop.height / (conf->downsize.height * conf->decimation.vertical_ratio);

  CMW_UTILS_get_down_config(ratiox, ratioy, conf->downsize.width, conf->downsize.height, down);
}
