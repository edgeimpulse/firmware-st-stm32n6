...
static int32_t CMW_VD66GY_getMirrorFlipConfig(uint32_t Config)
{
  switch (Config) {
    case CMW_MIRRORFLIP_NONE:
      return VD6G_MIRROR_FLIP_NONE;
      break;
...
static int32_t CMW_VD66GY_Init(void *io_ctx, CMW_Sensor_Init_t *initSensor)
{
  ...
  isp_stat_area.X0 = 0;
  isp_stat_area.Y0 = 0;
  isp_stat_area.XSize = initSensor->width;
  isp_stat_area.YSize = initSensor->height;
  ret = ISP_Init(&((CMW_VD66GY_t *)io_ctx)->hIsp, ((CMW_VD66GY_t *)io_ctx)->hdcmipp, 0, &((CMW_VD66GY_t *)io_ctx)->appliHelpers, &isp_stat_area);
  if (ret != ISP_OK)
  {
  ...
  ((CMW_VD66GY_t *)io_ctx)->width = initSensor->width;
  ((CMW_VD66GY_t *)io_ctx)->height = initSensor->height;
  return CMW_ERROR_NONE;
}
...
int32_t CMW_VD66GY_SetGain(void *io_ctx, int32_t gain)
{
  ...
  again_min_mdB = LINEAR_TO_MDECIBEL(32 / (32 - again_regmin));
  again_max_mdB = LINEAR_TO_MDECIBEL(32 / (32 - again_regmax));
  dgain_min_mdB = LINEAR_TO_MDECIBEL(FP58_TO_FLOAT(dgain_regmin));
  dgain_max_mdB = LINEAR_TO_MDECIBEL(FP58_TO_FLOAT(dgain_regmax));
  ...
  ret = VD6G_SetAnalogGain(&((CMW_VD66GY_t *)io_ctx)->ctx_driver, 32 - (32/analog_linear_gain));
  if (ret)
    return ret;
...
{
  return VD6G_SetExposureMode(&((CMW_VD66GY_t *)io_ctx)->ctx_driver, mode);
}
...
int32_t CMW_VD66GY_GetSensorInfo(void *io_ctx, ISP_SensorInfoTypeDef *info)
{
  ...
  /* Get resolution info */
  info->width = ((CMW_VD66GY_t *)io_ctx)->width;
  info->height = ((CMW_VD66GY_t *)io_ctx)->height;
