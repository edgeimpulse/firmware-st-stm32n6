...
ISP_StatusTypeDef ISP_Start(ISP_HandleTypeDef *hIsp)
{
  ...
  /* It is mandatory to ensure that RAW frame size does not exceed 2688 width prior demosaicing */
  ISP_DecimationTypeDef decimation;
  if ((SensorInfo.width / ISP_DECIM_FACTOR_1) < ISP_RAW_MAX_WIDTH)
  {
    decimation.factor = ISP_DECIM_FACTOR_1;
  }
  else if ((SensorInfo.width  / ISP_DECIM_FACTOR_2) < ISP_RAW_MAX_WIDTH)
  {
    decimation.factor = ISP_DECIM_FACTOR_2;
  }
  else if ((SensorInfo.width  / ISP_DECIM_FACTOR_4) < ISP_RAW_MAX_WIDTH)
  {
    decimation.factor = ISP_DECIM_FACTOR_4;
  }
  else if ((SensorInfo.width  / ISP_DECIM_FACTOR_8) < ISP_RAW_MAX_WIDTH)
  {
  ...
  ret = ISP_SVC_ISP_SetStatArea(hIsp, &hIsp->statArea, &decimation);
  if (ret != ISP_OK)
  {
    return ret;
  }
